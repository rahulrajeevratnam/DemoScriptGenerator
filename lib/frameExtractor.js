...
function extractFrames(videoPath, framesDir, log) {
  return new Promise((resolve, reject) => {
    ffmpeg(videoPath)
      .outputOptions(['-vf', "select='eq(n,0)+gt(scene,0.05)',setpts=N/FRAME_RATE/TB", '-vsync', 'vfr', '-q:v', '2'])
      .output(path.join(framesDir, 'frame_%03d.jpg'))
      .on('start', () => log('   ffmpeg started scene-change frame extraction (threshold 0.1)...'))
