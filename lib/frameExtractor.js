...
async function deduplicateFrames(frames, log) {
  ...
  const SIMILARITY_THRESHOLD = 0.80; // drop if >80% similar to previous
  ...
  function similarity(a, b) {
    let same = 0;
    const tolerance = 10; // per-channel tolerance
  ...
  for (let i = 1; i < frames.length; i++) {
    ...
    if (sim > SIMILARITY_THRESHOLD) {
      log(`   🗑  Dropped duplicate frame ${path.basename(frames[i])} (${Math.round(sim * 100)}% similar to previous)`);
    } else {
      log(`   ✅ Kept frame ${path.basename(frames[i])} (${Math.round((1 - sim) * 100)}% different from previous)`);
      kept.push(frames[i]);
