...
// ─── Per-batch prompt ──────────────────────────────────────────────────────
function batchPrompt(description, batchIdx, totalBatches, frameOffset) {
  if (batchIdx === 0) {
    return `The user recorded a demo of: "${description}"

These are frames ${frameOffset + 1} to ${frameOffset + 20} of the video (batch 1 of ${totalBatches}).

Analyse these frames and return a JSON array. Use the EXACT schema below. Respond with ONLY the JSON array:
...
[
  ...
  return `These are frames ${frameOffset + 1} onwards of the SAME demo (batch ${batchIdx + 1} of ${totalBatches}).

The user's demo description is: "${description}"

Analyse these NEW frames and return ONLY the NEW action steps you see that have not already been covered. Return a JSON array of section objects using the SAME schema. If these frames show steps within an existing sub-section, use the same sectionNumber/subSectionNumber. If they show a new process step, increment accordingly.

IMPORTANT: Start actionNumber from where the previous batch left off. Return ONLY valid JSON array, nothing else.`;
}
...
async function analyseVideo(frames, description, log) {
  ...
  for (let batchIdx = 0; batchIdx < batches.length; batchIdx++) {
    ...
    imageContent.push({ type: 'text', text: batchPrompt(description, batchIdx, batches.length, frameOffset) });
  ...
  const merged = mergeBatchResults(allBatchResults, description);
