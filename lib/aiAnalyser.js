...
- SKIP frames showing loading screens, spinners, progress bars, "Please wait" messages, page transitions, or any frame where the UI is not in a fully loaded, stable, interactive state. Do NOT document these as steps — they are noise.
- Only document frames where the UI is fully loaded and a meaningful user interaction is visible or has just completed (a form is filled, a button was clicked and the result is shown, a dialog has opened, a table has loaded with data, etc.).
- Do NOT create two consecutive actions that describe the same screen state or the same interaction. If multiple frames show the same UI state (e.g. hovering then clicking the same element, or the same menu open across two frames), document it as ONE action only. Each action must advance the screen to a new state.
...
- frameNumber must be the 1-based index of the frame that best shows the screen AFTER the action completes — always a stable, fully-loaded screen, never a loading or transition frame.
