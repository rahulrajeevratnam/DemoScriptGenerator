...
// ─── Action heading: "Action N - SubTitle: Step N" ────────────────────────
...
async function generateDocx({ scriptData, annotatedMap, description, templatePath, outputPath, log }) {
  ...
  for (const section of scriptData) {
    ...
    for (const subSection of section.subSections) {
      ...
      // ── Sub-sub-section / activity block ──────────────────────────────
      if (subSection.activityTitle) {
        children.push(subSubHeading(subSection.activityTitle));
      }
      ...
      for (const action of subSection.actions) {
        ...
        children.push(actionHeading(sectionActionCounter, subSection.subSectionTitle, action.stepNumber || sectionActionCounter));
